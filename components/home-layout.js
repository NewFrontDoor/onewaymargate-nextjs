...
const containerSx = (actions) => ({
	display: "grid",
	gridGap: "2em",
	gridTemplateColumns: ["1fr 100px 100px 1fr", `auto repeat(${actions}, 7.25rem) auto`],
...
const homeSectionBackground = ({ styling }) => ({
	backgroundColor: styling.style === "custom" ? styling.custom_color.hex : overlayColors[styling.style],
	width: "100%",
	position: "relative"
});
					...
					{actions && (
						<div sx={containerSx(actions.length)}>
							{actions.map((link, index) => {
								...
								return <Action key={link.url} link={link} column={index + 2} displaystyle={styling} />;
							})}
						</div>
					)}
				</div>
