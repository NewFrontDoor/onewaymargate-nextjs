...
import { pageQuery, menuQuery, sermonQuery, seriesQuery, defaultQuery } from "../../lib/queries";
	...
	return (
		<Layout menuData={menuData} mainData={pageData}>
			<article sx={main}>
				...
				<SermonGrid sermons={sermonData} series={seriesData} config={defaultData} />
				<div sx={{ overflowX: "scroll" }}>
					<SermonTable
						...
						renderLink={(directory, slug, title) => <Link link={`${directory}/${slug}`}>{title}</Link>}
					/>
				</div>
			</article>
		</Layout>
